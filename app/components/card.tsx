...
const AlbumCard = ({ album, itemVariants }: { album: any, itemVariants?: any }) => (
    <motion.div
        ...
        whileTap={{ scale: 0.98 }}
        className="flex items-center gap-5 p-5 rounded-[32px] bg-white/5 transition-all cursor-pointer group border border-white/5 border-white/20"
    >
        ...
        <motion.button 
            ...
            className="w-12 h-12 rounded-full bg-orange-500 text-white flex items-center justify-center opacity-0 opacity-100 transform translate-x-4 translate-x-0 transition-all shadow-lg shadow-orange-500/20"
        >
...
const VideoCard = ({ video, onClick, itemVariants }: { video: any, onClick: any, itemVariants?: any }) => (
    <motion.div 
      onClick={onClick}
      variants={itemVariants}
      whileHover={{ y: -8, scale: 1.02 }}
      className="group flex flex-col gap-4 cursor-pointer p-3 rounded-[32px] bg-white/5 transition-colors"
    >
      <div className="relative aspect-video rounded-[24px] overflow-hidden shadow-2xl shadow-black/50 ring-1 ring-white/5 ring-orange-500/50 transition-all duration-300">
        <img src={video.thumb} alt={video.title} loading="eager" className="w-full h-full transform group-hover:scale-110 transition-transform duration-700 ease-[cubic-bezier(0.25,0.1,0.25,1)]" />
        
        {/* Hover Overlay */}
        <div className="absolute inset-0 bg-black/40 opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
            <motion.div 
